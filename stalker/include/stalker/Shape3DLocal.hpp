...
inline void ShapeLocal<T>::estimNormal(){
	...
	norm_est.setInputCloud (this->_shape);
	norm_est.compute (*(this->_shape_normals));
}
...
inline void ShapeLocal<T>::downsample(){
	...
	std::cout << "Model total points: " << this->_shape->size () << "; Selected Keypoints: " << this->_shape_keypoints->size () << std::endl;
}
