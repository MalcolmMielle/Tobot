...
void publishTransform(const nav_msgs::Odometry& odomRead, tf::TransformBroadcaster& odom_broadcaster, Filter& filter){
...
int main(int argc, char **argv)
{
	...
	while(ros::ok()){
		...
		if(platform.getControl().getReadState()){
			...
			publishTransform(platform.getOdom(), odom_broadcaster, filt);
		}
		ros::spinOnce();
		loop_rate.sleep();
